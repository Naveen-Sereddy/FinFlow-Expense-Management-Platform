...
const SettingsSecurity = () => (
  <div className="ff-stack" style={{'--ff-stack-gap':'16px'}}>
    ...
    <div>
      <div className="ff-row" style={{padding:'0 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Active sessions</span></div>
      <table className="ff-table">
        <thead><tr><th>Device</th><th>Location</th><th>Last active</th><th></th></tr></thead>
        <tbody>
          <tr><td><Icon name="laptop" size={14}/> MacBook Pro · Chrome 128</td><td>San Francisco, CA</td><td className="ff-tnum">Now</td><td><StatusBadge status="active"/></td></tr>
...
const SettingsTeam = () => (
  <div>
    <div className="ff-row" style={{padding:'0 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Team & roles</span><div style={{flex:1}}/><button className="ff-btn ff-btn--sm ff-btn--primary"><Icon name="user-plus" size={12}/> Invite</button></div>
    <table className="ff-table">
...
const SettingsPolicies = () => {
  ...
  return (
    <div>
      <div className="ff-row" style={{padding:'0 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Policies</span><div style={{flex:1}}/><button className="ff-btn ff-btn--sm ff-btn--primary"><Icon name="plus" size={12}/> New policy</button></div>
      <table className="ff-table">
        ...
        <Card key={k}>
          <div className="ff-row" style={{justifyContent:'space-between', marginBottom:10}}>
            <div className="ff-row" style={{gap:10}}>
              <div style={{width:36, height:36, borderRadius:8, background:'var(--ff-card-2)', display:'grid', placeItems:'center', border:'1px solid var(--ff-border)'}}>
                {i.brand ? <BrandIcon name={i.brand} size={20}/> : <Icon name={i.icon} size={18}/>}
              </div>
              <div>
          ...
          <p style={{margin:'8px 0 14px', fontSize:13, color:'var(--ff-fg-muted)'}}>
            {{
              ...
              "Wise":"Pull statements; fund reimbursements from operating account.",
              "Brex":"Import card transactions and statements automatically.",
              "Xero":"Sync to Xero GL for monthly close.",
              "Google Chat":"Approve and review expenses inline in Google Chat.",
  ...
  <div className="ff-stack" style={{'--ff-stack-gap':'16px'}}>
      ...
      <div className="ff-row" style={{justifyContent:'space-between', alignItems:'flex-start'}}>
        <div>
          ...
          <div className="ff-row" style={{gap:6, marginTop:10}}>
            ...
            <span className="ff-badge ff-badge--neutral">Renews Jan 14, 2027</span>
          </div>
        </div>
        <div>
    ...
    <Card title="Payment method">
      <div className="ff-row" style={{justifyContent:'space-between', alignItems:'center'}}>
        <div className="ff-row" style={{gap:14, alignItems:'center'}}>
          <div style={{
            width:72, aspectRatio:'1.586 / 1', borderRadius:6, padding:'8px 9px',
            background:'linear-gradient(135deg, var(--ff-blue-700), var(--ff-blue-900))',
            display:'flex', flexDirection:'column', justifyContent:'space-between',
          }}>
            <div style={{width:14, height:10, borderRadius:2, background:'linear-gradient(135deg, #e8d48a, #c9a94f)'}}/>
            <BrandIcon name="visa" size={22} style={{filter:'brightness(0) invert(1)'}}/>
          </div>
          <div>
    ...
    <div>
      <div className="ff-row" style={{padding:'0 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Invoices</span></div>
      <table className="ff-table">
  ...
  <Card title="Notification preferences">
    {[
      ...
      <div key={i} className="ff-row" style={{padding:'12px 0', borderBottom: i < arr.length-1 ? '1px solid var(--ff-border)' : '0'}}>
        ...
        <label className="ff-row" style={{gap:6, fontSize:12, minWidth:80}}><input type="checkbox" defaultChecked={gc}/> Google Chat</label>
