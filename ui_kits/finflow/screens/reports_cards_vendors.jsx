  ...
  return (
    <>
      ...
      <div className="ff-grid" style={{gridTemplateColumns:'1fr 1fr', gap:16}}>
        <div className="ff-card" style={{padding:20, background:'oklch(0.16 0.01 240)', color:'#fff', display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
          <div style={{fontSize:11, letterSpacing:'.04em', textTransform:'uppercase', opacity:0.65}}>YTD Spend</div>
          <div style={{fontSize:40, fontWeight:700, letterSpacing:'-0.02em', margin:'6px 0'}}>$553.0K</div>
          <div style={{fontSize:12, opacity:0.75}}>+18.4% YoY</div>
        </div>
        <StatRow items={[
          { label: "Burn rate", value: "$108.6K", delta: "May avg, monthly" },
          { label: "Top vendor", value: "Google Cloud", delta: "$41.3K · 7.5% of YTD" },
          { label: "Open reports", value: "6", delta: "2 due this week" }
        ]}/>
      </div>
      <div className="ff-grid" style={{gridTemplateColumns:'1fr 1fr', marginTop:20}}>
        <Card title="Cumulative spend — FY26" action={<span className="ff-row" style={{gap:8, fontSize:12, color:'var(--ff-fg-muted)'}}><span style={{width:8, height:8, background:'var(--ff-chart-1)', borderRadius:2}}/>Actual spend</span>}>
          <AreaChart data={d.cumulative} height={220} unit="$" label="Cumulative spend for fiscal year 2026"/>
        </Card>
        <div>
          <div className="ff-row" style={{padding:'0 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Top vendors — YTD</span></div>
      ...
      <div className="ff-row" style={{padding:'20px 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Saved reports</span></div>
      <table className="ff-table">
  ...
  <>
    ...
    <div className="ff-grid" style={{gridTemplateColumns:'260px 1fr 280px', gap:16}}>
      ...
      <Card title="Preview">
        ...
        <BarChart data={FF_DATA.categoryBreakdown.map(c=>({cat: FF_DATA.categories.find(x=>x.id===c.cat).name, value:c.value}))}
          colors={FF_DATA.categoryBreakdown.map(c => FF_DATA.categories.find(x=>x.id===c.cat).color)}
          height={260} unit="$" label="Spend by category, last 90 days"/>
      </Card>
      <Card title="Options">
...
const SavedReport = () => {
  ...
  return (
    <>
      ...
      <StatRow items={[
        { label: "Total spend", value: "$348.0K", delta: "+9% vs Mar", trend: "up" },
        { label: "Reimbursed", value: "$42.1K", delta: "142 payouts" },
        { label: "Variance vs plan", value: "−2.4%", delta: "Under by $8.6K", trend: "up" },
        { label: "Top category", value: "Software", delta: "$184.2K · 33%" }
      ]} style={{padding:'4px 0 20px'}}/>
      <div className="ff-grid" style={{gridTemplateColumns:'1.4fr 1fr'}}>
        <Card title="Spend by week">
          <LineChart data={d.spendOverTime} height={240} unit="$" label="Weekly spend for April close"/>
        </Card>
        <Card title="By category">
          <BarChart data={d.categoryBreakdown.map(c => ({ cat: d.categories.find(x=>x.id===c.cat).name, value: c.value }))}
            colors={d.categoryBreakdown.map(c => d.categories.find(x=>x.id===c.cat).color)}
            height={240} unit="$" label="April spend by category"/>
        </Card>
      </div>
    </>
  );
...
const CardsList = () => {
  ...
  return (
    <>
      ...
      {d.cards.length === 0 ? (
        <EmptyState icon="credit-card" title="No cards issued yet"
          body="Issue a virtual card instantly, or order a physical card for a teammate."
          actions={<button className="ff-btn ff-btn--primary" onClick={()=>ffGo('issue-card')}><Icon name="plus" size={14}/> Issue card</button>}/>
      ) : (
      <div className="ff-grid" style={{gridTemplateColumns:'repeat(3, 1fr)', gap:16}}>
        {d.cards.map(c => (
          <div key={c.id} className="ff-card" style={{overflow:'hidden'}}>
            <div style={{
              padding:'20px', color:'#fff', aspectRatio:'1.586 / 1',
              background: c.status === 'frozen'
                ? 'linear-gradient(135deg, var(--ff-ink-700), var(--ff-ink-950))'
                : 'linear-gradient(135deg, var(--ff-blue-700), var(--ff-blue-900))',
              display:'flex', flexDirection:'column', justifyContent:'space-between', gap:12, position:'relative'
            }}>
              ...
              {c.type !== "virtual" && <div style={{width:34, height:26, borderRadius:5, background:'linear-gradient(135deg, #e8d48a, #c9a94f)'}}/>}
              ...
              <div className="ff-row" style={{justifyContent:'space-between', alignItems:'flex-end'}}>
                ...
                {c.status === 'frozen' && <span className="ff-badge ff-badge--on-dark"><span className="ff-badge__icon"><Icon name="snowflake" size={11}/></span>Frozen</span>}
              </div>
            </div>
            <div style={{padding:'14px 16px', borderTop:'1px solid var(--ff-border)', background:'var(--ff-card-2)'}}>
    ...
    <>
      ...
      <div className="ff-grid" style={{gridTemplateColumns:'1fr 1fr', gap:16}}>
        <Card title="This card">
          <div style={{padding:'24px', borderRadius:12, maxWidth:360, aspectRatio:'1.586 / 1', display:'flex', flexDirection:'column', justifyContent:'space-between', background:'linear-gradient(135deg, var(--ff-blue-700), var(--ff-blue-900))', color:'#fff'}}>
            ...
            <div className="ff-mono" style={{fontSize:20, letterSpacing:'0.12em'}}>•••• •••• •••• {c.last4}</div>
            <div className="ff-row" style={{justifyContent:'space-between'}}>
      ...
      <div className="ff-row" style={{padding:'20px 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Recent transactions</span></div>
      <table className="ff-table">
        ...
        <tbody>
          {d.expenses.filter(e => e.cardLast4 === c.last4).map(e => (
            <tr key={e.id}>
              ...
              <td><span className="ff-row" style={{gap:8}}><MerchantIcon name={e.merchant} size={14}/><strong>{e.merchant}</strong></span></td>
    ...
    <div className="ff-grid" style={{gridTemplateColumns:'1.4fr 1fr', gap:24}}>
      <Card>
        <div className="ff-stack" style={{'--ff-stack-gap':'14px'}}>
          ...
          <div className="ff-row" style={{justifyContent:'space-between'}}>
            <div><div style={{fontSize:13, fontWeight:500}}>Notify on each charge</div><div style={{fontSize:12, color:'var(--ff-fg-muted)'}}>Email + Google Chat</div></div>
      ...
      <Card title="Preview">
        <div style={{padding:'24px', borderRadius:12, maxWidth:360, aspectRatio:'1.586 / 1', display:'flex', flexDirection:'column', justifyContent:'space-between', background:'linear-gradient(135deg, var(--ff-blue-700), var(--ff-blue-900))', color:'#fff'}}>
          ...
          <div className="ff-mono" style={{fontSize:18, letterSpacing:'0.12em'}}>•••• •••• •••• ••••</div>
          <div className="ff-row" style={{justifyContent:'space-between'}}>
...
const VendorsList = () => {
  ...
  return (
    <>
      ...
      {/* Search-first composition — a directory screen starts with finding
          a row, not with a card wrapping the whole table. */}
      <div className="ff-search" style={{maxWidth:340, marginBottom:20}}>
      ...
      {d.vendors.length === 0 ? (
        <EmptyState icon="storefront" title="No vendors yet"
          body="Vendors appear here once you approve an expense from a new merchant, or add one directly."
          actions={<button className="ff-btn ff-btn--primary"><Icon name="plus" size={14}/> Add vendor</button>}/>
      ) : (
        <table className="ff-table">
...
const VendorDetail = () => {
  ...
  return (
    <>
      <PageHead eyebrow="Vendor" title={<span className="ff-row" style={{gap:10, alignItems:'center'}}><MerchantIcon name={v.name} size={26}/> {v.name}</span>} sub="EIN 47-2210331 · Tax type C-Corp · Net-30"
        actions={<><button className="ff-btn"><Icon name="paperclip" size={14}/> Contracts</button><button className="ff-btn"><Icon name="envelope" size={14}/> Message</button><button className="ff-btn ff-btn--primary" onClick={()=>ffGo('schedule-payout')}>Pay vendor</button></>}/>
      <StatRow items={[
        { label: "YTD spend", value: `$${(v.spend/1000).toFixed(1)}K`, delta: `${v.change>0?'+':''}${v.change}% vs Q1`, trend: v.change > 0 ? "up" : v.change < 0 ? "down" : undefined },
        { label: "Transactions", value: "38", delta: "3 this week" },
        { label: "Avg invoice", value: "$1,086", delta: "−4% vs Q1" },
        { label: "Open invoices", value: "2", delta: "Both within 30d" }
      ]} style={{padding:'4px 0 20px'}}/>
      <Card title="Spend over time">
        <LineChart data={vendorTrend} height={220} unit="$" label={`Spend over time with ${v.name}`}/>
      </Card>
      <div className="ff-row" style={{padding:'20px 0 10px'}}><span style={{fontSize:15, fontWeight:600}}>Recent transactions</span></div>
      <table className="ff-table">
